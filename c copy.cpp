...
int main()
{
    ...
    /*
        tGameSituation test;
        start(test.board,10,10,hole);
        test.player.x = 1;
        test.player.y = 1;
        test.player.flavor = none;
        test.board.cells[1][1] = plain;
        test.board.cells[1][2] = plain;
        test.board.cells[2][3] = electric;
        test.board.cells[1][3] = water;
        test.board.cells[1][4] = goal;

        auto ret = solution(test);

        printInstructions(ret);
        printGame(test);
        cout << "valid: "<<ret.valid << endl;
        return 0;
    */
